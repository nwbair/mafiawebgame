...
if(strlen($_POST['Username'])<3 || strlen($_POST['Username'])>32)
	...
	if(empty($_POST['Agree'])){ // Check if the Checkbox is checked to agree with the terms of services
		...
		$sql = "SELECT id FROM users WHERE mail='".mysqli_real_escape_string($connection, $_POST['Email'])."'";
		$query = mysqli_query($connection, $sql) or die(mysqli_error());
		$m_count = mysqli_num_rows($query);
		...
		if($m_count >= "1"){
			...
			$sql = "SELECT id FROM users WHERE name='".mysqli_real_escape_string($connection, $_POST['Username'])."'";
			$query = mysqli_query($connection, $sql) or die(mysqli_error());
			$m_count = mysqli_num_rows($query);
...
Your Password: ".$_POST['Password']."<p>
    ...
    $headers  = "From: Game Registration Details <no-reply@email.com>\r\n";
